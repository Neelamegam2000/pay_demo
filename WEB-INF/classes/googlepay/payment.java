	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try{
		   ...
		   // s.close
			...
			String sql2="select balance from accounts where upi_id='"+receiver+"' and active_status='active';";
			...
			if(rs1.getInt("balance")>=amount){
				...
				ps1.executeUpdate();
				ps2.executeUpdate();
				String sql3="select category_id from category where upi_id='"+receiver+"';";
				ResultSet rs3=null;
				...
				if(s1.equals("null")){
					...
					Integer reward_amount=(int)(((Math.random())*10)+1);
				    ps4.setInt(1,reward_id);
				    ps4.setInt(2, transaction_id);
				    ps4.setInt(3,reward_amount);
				    ps4.setString(4, null);
				    ps4.executeUpdate();
				    String sql5="select balance from accounts where upi_id='"+sender_upi_id+"' and active_status='active';";
				    ResultSet rs5=s.executeQuery(sql5);
					rs5.next();
					Integer sender_balance1=rs5.getInt("balance")+reward_amount;
					ps1.setInt(1,sender_balance1 );
					ps1.setString(2, sender_upi_id);
					ps1.executeUpdate();
